...
const ImplementationGantt = () => {
  ...
  const [taskOrder, setTaskOrder] = useState({}); // Track custom task ordering
  ...
  // Function to recalculate dependent task positions
  const recalculateDependencies = useCallback((tasks, changedTaskId) => {
    ...
    const dependencies = {
      'requirements': ['kickoff'], // Requirements starts when kickoff completes
      'recruiting-setup': ['requirements'], // Module setups start after requirements
      'onboarding-setup': ['recruiting-implementation'], // Onboarding starts when recruiting implementation starts
      'lms-setup': ['onboarding-implementation'], // LMS starts when onboarding implementation starts
      'performance/goals/engagement-setup': ['lms-implementation'], // Performance starts when LMS implementation starts
      'recruiting-testing': ['recruiting-setup'], // Testing starts after setup
      'onboarding-testing': ['onboarding-setup'],
      'lms-testing': ['lms-setup'],
      'performance/goals/engagement-testing': ['performance/goals/engagement-setup'],
      'historical-data-import': ['recruiting-implementation'], // Data import is part of recruiting
      'recruiting-integration': ['recruiting-implementation'],
      'onboarding-integration': ['onboarding-implementation'],
      'rollout-training': [], // Launch phase starts after all execution tasks
      'golive': ['rollout-training'] // Go Live starts after rollout training
    };
    
    // Function to update dependent tasks recursively
    const updateDependentTasks = (taskId) => {
      const task = updatedTasks.find(t => t.id === taskId);
      if (!task) return;
      
      // Find all tasks that depend on this task
      Object.entries(dependencies).forEach(([dependentId, prereqs]) => {
        if (prereqs.includes(taskId)) {
          const dependentTask = updatedTasks.find(t => t.id === dependentId);
          if (dependentTask) {
            // Calculate new start time based on prerequisite completion
            const newStart = task.start + task.duration;
            if (dependentTask.start !== newStart) {
              dependentTask.start = newStart;
              // Recursively update tasks that depend on this one
              updateDependentTasks(dependentId);
            }
          }
        }
      });
      
      // Special handling for module sub-tasks
      if (task.id.includes('-implementation')) {
        const moduleType = task.id.replace('-implementation', '');
        
        // Update setup task to start with implementation
        const setupTask = updatedTasks.find(t => t.id === `${moduleType}-setup`);
        if (setupTask) {
          setupTask.start = task.start;
        }
        
        // Update learning task to start with implementation
        const learningTask = updatedTasks.find(t => t.id === `${moduleType}-learning`);
        if (learningTask) {
          learningTask.start = task.start;
        }
        
        // Update testing to start after setup completes
        const testingTask = updatedTasks.find(t => t.id === `${moduleType}-testing`);
        if (testingTask && setupTask) {
          testingTask.start = setupTask.start + setupTask.duration;
        }
        
        // Update integration and data import to end with implementation
        const integrationTask = updatedTasks.find(t => t.id === `${moduleType}-integration`);
        if (integrationTask) {
          integrationTask.start = task.start + task.duration - integrationTask.duration;
        }
        
        if (moduleType === 'recruiting') {
          const dataImportTask = updatedTasks.find(t => t.id === 'historical-data-import');
          if (dataImportTask) {
            dataImportTask.start = task.start + task.duration - dataImportTask.duration;
          }
        }
      }
    };
    
    // Update all dependent tasks starting from the changed task
    updateDependentTasks(changedTaskId);
  ...
  // Update timeline state when timeline changes
  useEffect(() => {
  ...
  const totalWeeks = useMemo(() => {
    ...
    const lastTask = timelineState.length > 0 ? 
      timelineState.reduce((latest, task) => {
  ...
  // Group tasks by phase with custom ordering
  const tasksByPhase = useMemo(() => {
    const grouped = {};
    const phases = ['Initiation & Planning', 'Execution', 'Launch'];
    
    phases.forEach(phase => {
      const phaseTasks = timelineState.filter(task => task.phase === phase);
      // Sort by custom order, then by start time
      phaseTasks.sort((a, b) => {
          ...
          title={
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <Typography variant="body2" sx={{ color: colors.dark, fontStyle: 'italic' }}>
                {tierInfo.package !== 'ClearCare Pro' ? 'Drag tasks horizontally to adjust timing, vertically to reorder. Drag right edge to resize.' : 'Self-paced implementation - no drag functionality'}
              </Typography>
            </Box>
          }
        />
                  ...
                  <Box key={phase} sx={{ position: 'relative', mb: 3 }}>
                    ...
                    {phaseTasks.map(task => {
                      ...
                      const barText = task.isSelfPaced ? task.selfPacedLabel : 
                                     (task.duration >= 0.5 ? `${Math.round(task.duration * 10) / 10}w` : '');
